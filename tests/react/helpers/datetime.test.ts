...
describe("DateTime Helpers", () => {
  describe("fromInputParts", () => {
    it("should correctly parse standard date and time", () => {
      ...
      // Paris in May is CEST (UTC+2), so 10:00 Paris -> 08:00 UTC
  ...
  describe("toInputDate", () => {
    it("should format date as YYYY-MM-DD", () => {
      // Use UTC to create a stable date: 10:00 UTC is 12:00 Paris
      const date = new Date(Date.UTC(2026, 4, 5, 10, 0));
      expect(toInputDate(date)).toBe("2026-05-05");
    });

    it("should accept an ISO string as input", () => {
      const iso = "2026-05-05T00:00:00.000Z";
      // In Paris (UTC+2), this is 2026-05-05 at 02:00
  ...
  describe("toInputTime", () => {
    it("should format time as HH:mm", () => {
      ...
      const date = new Date(Date.UTC(2026, 4, 5, 9, 5));
      expect(toInputTime(date)).toBe("11:05");
    });
  ...
  describe("toDisplayString", () => {
    it("should format date and time as localized string", () => {
      const date = new Date(Date.UTC(2026, 4, 5, 9, 5));
      // 09:05 UTC -> 11:05 Paris
      expect(toDisplayString(date)).toContain("5 mai 2026");
      expect(toDisplayString(date)).toContain("11:05");
    });
